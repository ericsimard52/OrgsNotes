...
**** More common way to test
     Interger comparison with single bracket
     The results of the following script can be a little confusing. What is important to notice and remember:
...
#+NANE HEADER
#+BEGIN_SRC bash :results output :tangle myFm.sh :tangle-mode identity(#o0755) :cmdline 2>&1
  #!/bin/bash
  declare -f enterDir existDir listContent createFile removeFile createDirectory removeDirectory Quit
#+END_SRC

#+NAME: exitDirectory
#+BEGIN_SRC bash :results output :tangle myFm.sh :tangle-mode identity(#o0755) :shebang :cmdline 2>&1 # :epilogue existDirectory
...
#+NAME: enterDirectory
#+BEGIN_SRC bash :results output :tangle myFm.sh :tangle-mode identity(#o0755) :shebang :cmdline #:epilogue enterDirextory ~/
  function enterDirextory {
      echo 1
      if [ -z $1 ]; then # No directory provided
          return 1
      fi
      ...
      pushd $1 #>/dev/null # Place directory on top of the stack.
  }
...
#+NAME: listContent
#+BEGIN_SRC bash :results output :tangle myFm.sh :tangle-mode identity(#o0755) :shebang :noweb yes :cmdline 2>&1 # :epilogue listContent ~/
...
#+BEGIN_SRC bash :results output :tangle myFm.sh :tangle-mode identity(#o0755) :shebang :noweb yes :cmdline 2>&1 # :epilogue listContent ~/
  function createFile {
      ...
      eval "touch $1" # Attempt to create the file
      if (( $1 > 0 )); then # Return value of eval is stored in $?
          # >&2 redirects output into stderr.
          # It is good practice to redirect error message to stderr (file descriptor 2)
          >&2 echo "eval touch $1 returned an error."
...
#+BEGIN_SRC bash :results output :tangle myFm.sh :tangle-mode identity(#o0755) :shebang :noweb yes :cmdline 2>&1 # :epilogue listContent ~/
  function removeFile {
      select file in * "QUIT"; do
          ...
          if [ -d $file ]; then
              echo "$file is a directory, doing nothing."
              return 2
          fi
          if [ -f $file ]; then
              echo "\e[33mWARNING THIS WILL REALLY DELETE THE FILE.\e[0m"
              eval "rm -i $file"
              if [ $? > 0 ]; then
                  echo "Removal of the file caused an error."
                  return 3
              fi
